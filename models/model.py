...
class Model(nn.Module):
    ...
    def policy(self, obs, schedule=None, last_act=None, last_hid=None, info={}, stat={}):
        ...
        if self.args.agent_id:
            agent_ids = torch.eye(self.n_).unsqueeze(0).repeat(batch_size, 1, 1).to(self.device) # shape = (b, n, n)
        ...
        if self.args.shared_params:
            ...
            if self.args.gaussian_policy:
                ...
                stds = torch.ones_like(means).to(self.device) * self.args.fixed_policy_std
            ...
            if self.args.gaussian_policy:
                ...
                log_stds = torch.zeros_like(means).to(self.device)
    ...
    def credit_assignment_demo(self, obs, act):
        ...
        act = torch.tensor(act).to(self.device).float()
    ...
    def unpack_data(self, batch):
        ...
        last_step = torch.tensor(batch.last_step, dtype=torch.float).to(self.device).contiguous().view(-1, 1)
        done = torch.tensor(batch.done, dtype=torch.float).to(self.device).contiguous().view(-1, 1)
        action = torch.tensor(np.concatenate(batch.action, axis=0), dtype=torch.float).to(self.device)
        log_prob_a = torch.tensor(np.concatenate(batch.action, axis=0), dtype=torch.float).to(self.device)
        value = torch.tensor(np.concatenate(batch.value, axis=0), dtype=torch.float).to(self.device)
        next_value = torch.tensor(np.concatenate(batch.next_value, axis=0), dtype=torch.float).to(self.device)
        state = prep_obs( list( zip(batch.state) ) ).to(self.device)
        next_state = prep_obs( list( zip(batch.next_state) ) ).to(self.device)
        action_avail = torch.tensor( np.concatenate(batch.action_avail, axis=0) ).to(self.device)
        last_hid = torch.tensor(np.concatenate(batch.last_hid, axis=0), dtype=torch.float).to(self.device)
        hid = torch.tensor(np.concatenate(batch.hid, axis=0), dtype=torch.float).to(self.device)
